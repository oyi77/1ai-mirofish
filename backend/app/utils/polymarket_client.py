...
class PolymarketClient:
    ...
    async def find_market(self, question: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            resp = await client.get(
                f"{self.gamma_url}/markets", params={"question": question, "limit": 5}
            )
            resp.raise_for_status()
            markets = resp.json()
    ...
    async def get_market_price(self, condition_id: str) -> Optional[float]:
        ...
        try:
            resp = await client.get(
                f"{self.gamma_url}/markets", params={"conditionId": condition_id}
            )
            resp.raise_for_status()
            markets = resp.json()
            if not markets:
    ...
    async def get_order_book(self, token_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            resp = await client.get(
                f"{self.clob_url}/order_book", params={"token_id": token_id}
            )
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
    ...
    async def analyze_liquidity(self, token_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get market data
            resp = await client.get(
                f"{self.gamma_url}/markets", params={"tokenId": token_id}
            )
            resp.raise_for_status()
            markets = resp.json()
            ...
            logger.info(
                f"Liquidity analysis: spread={result['spread']}, vol={result['volume_24h']}"
            )
    ...
    async def get_whale_positions(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            resp = await client.get(
                f"{self.data_url}/leaderboard", params={"window": "30d", "limit": top_n}
            )
            resp.raise_for_status()
            return resp.json()
    ...
    async def get_market_history(self, condition_id: str) -> Dict[str, Any]:
        ...
        result = {"resolved": False, "outcome": None, "end_date": None}
            ...
            if markets:
                ...
                if result["resolved"]:
                    ...
                    if isinstance(outcome, dict):
                        result["outcome"] = outcome.get(
                            "title", outcome.get("winner", "Unknown")
                        )
                    else:
    ...
    async def get_market_stats(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Market stats for '{question[:40]}': price={stats['current_price']}, vol={stats['volume']}"
            )
