...
def prepare_simulation():
    ...
    try:
        ...
        if not simulation_id:
            return (
                jsonify({"success": False, "error": t("api.requireSimulationId")}),
                400,
            )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task_id:
            ...
            return (
                jsonify({"success": False, "error": t("api.requireTaskOrSimId")}),
                400,
            )
        ...
        if not task:
            ...
            return (
                jsonify({"success": False, "error": t("api.taskNotFound", id=task_id)}),
                404,
            )
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return (
                jsonify({"success": False, "error": t("api.configFileNotFound")}),
                404,
            )
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return (
                jsonify({"success": False, "error": t("api.noMatchingEntities")}),
                400,
            )
    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                return (
                    jsonify({"success": False, "error": t("api.maxRoundsInvalid")}),
                    400,
                )
...
def interview_agent():
    ...
    try:
        ...
        if platform and platform not in ("twitter", "reddit"):
            return (
                jsonify({"success": False, "error": t("api.invalidInterviewPlatform")}),
                400,
            )
...
# ============== PolyEdge Signal Integration ==============


@simulation_bp.route("/signals", methods=["GET"])
def get_trading_signals():
    """
    Generate trading signals from graph knowledge for PolyEdge integration.

    Query parameters:
        market: Market platform filter (default: "polymarket")
        question: Market question context (optional)
        market_price: Current market price 0.0-1.0 (optional)

    Returns:
        {"signals": [{"market_id": "...", "prediction": 0.0-1.0, "confidence": 0.0-1.0, "reasoning": "...", "source": "mirofish"}]}
    """
    try:
        market = request.args.get("market", "polymarket")
        question = request.args.get("question", "")
        market_price = request.args.get("market_price", "0.5")

        if not Config.LLM_API_KEY:
            return jsonify({"signals": [], "error": "LLM not configured"}), 500
        ...
        prompt = f"""You are a trading signal generator. Based on the knowledge graph context below, generate trading signals for {market}.

Market question: {question or 'General market analysis'}
Current market price: {market_price}

Knowledge Graph Context:
{graph_context}

Generate 1-3 trading signals as JSON array. Each signal:
- market_id: unique identifier
- prediction: float 0.0-1.0 (probability of YES/outcome)
...
Return ONLY valid JSON array, no markdown."""
        ...
        logger.info(f"LLM response length: {len(content)}, model: {Config.LLM_MODEL_NAME}")

        if not content:
            logger.warning(f"Empty LLM response. Full response: {resp_json}")
            return jsonify({"signals": [], "error": "Empty LLM response"})
        ...
        formatted_signals = []
        for s in signals:
            if all(k in s for k in ["market_id", "prediction", "confidence"]):
                formatted_signals.append(
                    {
                        "market_id": str(s["market_id"]),
                        "prediction": max(0.0, min(1.0, float(s["prediction"]))),
